use crate::{ContentDecoded, ContentEncoding, EmailBody, Part};
...
fn parse_part(mut part: Part, email: &mut EmailBody) -> Result<(), &'static str> {
    ...
    match part.content_features.get("Content-Transfer-Encoding") {
        Some(v) => {
            ...
            } else if v.contains("quoted-printable") {
                encoding = ContentEncoding::Qp;
            } else if v.contains("qp") {
                encoding = ContentEncoding::Qp;
            } else if v.contains("text") {
                encoding = ContentEncoding::String;
            } else if v.contains("string") {
                encoding = ContentEncoding::String;
            } else if v.contains("7bit") {
                encoding = ContentEncoding::String;
            } else if v.contains("8bit") {
                encoding = ContentEncoding::String;
            } else if v.contains("binary") {
                encoding = ContentEncoding::UnSupported;
            } else {
                encoding = ContentEncoding::String;
            }
        }
        None => match email.headers.get("Content-Transfer-Encoding") {
            Some(v) => {
                let v = v.to_lowercase();
                if v.contains("base64") {
                    encoding = ContentEncoding::Base64;
    ...
    match encoding {
        ...
        ContentEncoding::Qp => match QPDecode(part.data.clone(), QpParseMode::Strict) {
            Ok(v) => {
                if is_string {
                    match String::from_utf8(v) {
                        Ok(v) => {
